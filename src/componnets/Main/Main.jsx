...
const Main = () => {
  ...
  function onDragEnd(result) {
    ...
    const s_id = parseInt(source.droppableId, 10);
    const d_id = parseInt(destination.droppableId, 10);
    const [removed] = newList[s_id - 1].items.splice(source.index, 1);
    newList[d_id - 1].items.splice(destination.index, 0, removed);
  ...
  const listData = (e) => {
    let newList = [...bdata.list];
    newList.push({
      id: (newList.length + 1).toString(), // Convert ID to string
      title: e,
      items: []
    });
        ...
        <div className='absolute mb-1 pb-2 left-0 right-0 top-0 bottom-0 p-3 flex overflow-x-scroll overflow-y-hidden'>
          <DragDropContext onDragEnd={onDragEnd}>
            {bdata.list && bdata.list.map((x, ind) => (
              <div key={x.id} className='mr-3 w-60 h-fit rounded-md p-2 bg-black flex-shrink-0'>
                <div className="list-body">
                  ...
                  <Card getcard={(e) => cardData(e, ind)}></Card>
                </div>
              </div>
            ))}
          </DragDropContext>
